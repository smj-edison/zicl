 ...
 * 
 *   THIS IS AN ALTERED VERSION. It was altered by Mason Jones
 *   (https://github.com/smj-edison) on 29 April 2026 to use Zig-friendly
 *   allocation.
 *
...
/* --- Zig-provided shims --- */
/* UTF-8 */
extern int ziclcompat_utf8_tounicode(const char *bytes, int *uc);
extern int ziclcompat_utf8_upper(int uc);
extern int ziclcompat_utf8_charlen(int c);
extern int ziclcompat_utf8_index(const char *s, int n);
extern int ziclcompat_utf8_getchars(char *buf, int cp);

/* Allocation (size-aware) */
extern void *ziclcompat_malloc(size_t n);
extern void *ziclcompat_realloc(void *ptr, size_t last_len, size_t new_len);
extern void ziclcompat_free(void *ptr, size_t len);
extern char *ziclcompat_strdup(const char *s);

/* The following is vendored from musl. */
...
/* Doesn't implement full behavior, just base 10. */
static unsigned long strtoul(const char *s, char **end) {
	unsigned long n = 0;
	while (*s == ' ' || *s == '\t' || *s == '\r' || *s == '\n') s++;
	while (*s >= '0' && *s <= '9') {
		n = n * 10 + (*s++ - '0');
...
/* End of shims and start of regex. */
...
static char *reg_expanded_new_pattern(const char *exp)
{
	...
	char *newexp = ziclcompat_strdup(exp);
	char *d = newexp;
	const char *s = exp;
	int escape = 0;
 ...
 */
int zicl_regcomp(regex_t *preg, const char *exp, int cflags)
{
	int scan;
	int longest;
	unsigned len;
	...
	preg->program = ziclcompat_malloc(preg->proglen * sizeof(int));
...
static int regpiece(regex_t *preg, int *flagp)
{
	...
	if (op == '{') {
		...
		min = strtoul(preg->regparse + 1, &end);
		...
		if (*end == '}') {
			...
			preg->regparse = end;
			max = strtoul(preg->regparse + 1, &end);
			if (*end != '}') {
				preg->err = REG_ERR_UNMATCHED_BRACES;
				return 0;
			}
...
{
	int l = ziclcompat_utf8_tounicode(s, uc);
	if (upper) {
		*uc = ziclcompat_utf8_upper(*uc);
	}
	return l;
}
	...
	case '[': {
			...
			while (*pattern != ']') {
				...
				if (pattern[0] == '-' && pattern[1] && pattern[1] != ']') {
					/* skip '-' */
					pattern += ziclcompat_utf8_tounicode(pattern, &end);
...
static void reg_grow(regex_t *preg, int n)
{
	if (preg->p + n >= preg->proglen) {
		int oldlen = preg->proglen;
		preg->proglen = (preg->p + n) * 2;
		preg->program = ziclcompat_realloc(preg->program, oldlen, preg->proglen * sizeof(int));
	}
}
 ...
 */
int zicl_regexec(regex_t  *preg,  const  char *string, size_t nmatch, regmatch_t pmatch[], int eflags)
{
	const char *s;
	int scan;
	...
	if (preg->regstart != '\0') {
		...
		while (1) {
			...
			if (*s == '\0') {
				...
				int c;
				s += ziclcompat_utf8_tounicode(s, &c);
			}
		}
...
static int regmatchsimplerepeat(regex_t *preg, int scan, int matchmin)
{
	...
	while (1) {
		...
		preg->reginput = save + ziclcompat_utf8_index(save, no);
...
static int regrepeat(regex_t *preg, int p, int max)
{
	...
	case ANY:
		while (!reg_iseol(preg, *scan) && count < max) {
			count++;
			scan += ziclcompat_utf8_charlen(*scan);
		}
		break;
	case EXACTLY:
		while (count < max) {
...
size_t zicl_regerror(int errcode, const regex_t *preg, char *errbuf,  size_t errbuf_size)
{
...
void zicl_regfree(regex_t *preg)
{
	if (preg->exp != NULL) {
		ziclcompat_free(preg->exp, strlen(preg->exp) + 1);
	}
	ziclcompat_free(preg->program, preg->proglen * sizeof(int));
}
