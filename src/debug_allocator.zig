...
pub fn DebugAllocator(comptime config: Config) type {
    return struct {
        ...
        /// One live (leaked) allocation surfaced by `iterateLeaks`.
        /// `addr` is the user-visible allocation address, `size` is the size
        /// class for small allocations or `bytes.len` for large ones, and
        /// `stack_trace` is the alloc-time capture (empty when
        /// `config.stack_trace_frames == 0`).
        pub const LeakEntry = struct {
            addr: usize,
            size: usize,
            stack_trace: StackTrace,
        };

        /// Lazy iterator over every still-live allocation in this allocator.
        /// Walks the small size-class buckets first (reusing the same
        /// used-bits and address reconstruction as `detectLeaksInBucket`),
        /// then the large allocation table. Yields only leaks: freed entries
        /// are skipped (when `retain_metadata` keeps them around at all).
        pub const LeakIterator = struct {
            alloc: *Self,
            size_class_index: usize = 0,
            bucket: ?*BucketHeader = null,
            used_bits_byte: usize = 0,
            bit_index: usize = 0,
            large_it: LargeAllocTable.ValueIterator,

            pub fn next(self: *LeakIterator) ?LeakEntry {
                // Small allocations: one size class per power-of-two bucket.
                while (self.size_class_index < small_bucket_count) {
                    if (self.bucket == null) {
                        self.bucket = self.alloc.buckets[self.size_class_index];
                        self.used_bits_byte = 0;
                        self.bit_index = 0;
                    }
                    while (self.bucket) |bucket| {
                        const slot_count = slot_counts[self.size_class_index];
                        const used_bits_count = usedBitsCount(slot_count);
                        while (self.used_bits_byte < used_bits_count) {
                            const used_int = bucket.usedBits(self.used_bits_byte).*;
                            if (used_int != 0) {
                                while (self.bit_index < @bitSizeOf(usize)) {
                                    const bit_shift: Log2USize = @intCast(self.bit_index);
                                    const is_used = @as(u1, @truncate(used_int >> bit_shift)) != 0;
                                    self.bit_index += 1;
                                    if (!is_used) continue;
                                    const slot_index: SlotIndex = @intCast(
                                        self.used_bits_byte * @bitSizeOf(usize) + bit_shift,
                                    );
                                    const size_class: usize = @as(usize, 1) << @as(Log2USize, @intCast(self.size_class_index));
                                    const page_addr = @intFromPtr(bucket) & ~(page_size - 1);
                                    return .{
                                        .addr = page_addr + @as(usize, slot_index) * size_class,
                                        .size = size_class,
                                        .stack_trace = bucketStackTrace(bucket, slot_count, slot_index, .alloc),
                                    };
                                }
                            }
                            self.used_bits_byte += 1;
                            self.bit_index = 0;
                        }
                        // Bucket chain exhausted; follow `prev` to the next bucket.
                        self.bucket = bucket.prev;
                        self.used_bits_byte = 0;
                        self.bit_index = 0;
                    }
                    // Size class exhausted; advance to the next one.
                    self.size_class_index += 1;
                }

                // Large allocations: one hashmap entry per allocation.
                while (self.large_it.next()) |large_alloc| {
                    if (config.retain_metadata and large_alloc.freed) continue;
                    return .{
                        .addr = @intFromPtr(large_alloc.bytes.ptr),
                        .size = large_alloc.bytes.len,
                        .stack_trace = large_alloc.getStackTrace(.alloc),
                    };
                }
                return null;
            }
        ...
        /// Returns an iterator over every live (leaked) allocation. Cheap to
        /// construct; the walk happens as `next` is called. Call this in place
        /// of `detectLeaks` when you want the leaked addresses and their
        /// alloc-time stack traces rather than a printed log.
        pub fn iterateLeaks(self: *Self) LeakIterator {
            return .{ .alloc = self, .large_it = self.large_allocations.valueIterator() };
        }
...
test "iterateLeaks reports live small and large allocations" {
    var gpa: DebugAllocator(.{ .stack_trace_frames = 6 }) = .{};
    const allocator = gpa.allocator();

    // One freed (must not appear), one leaked small, one leaked large.
    const small_freed = try allocator.alloc(u8, 8);
    allocator.free(small_freed);

    const small_leaked = try allocator.alloc(u8, 8);
    const large_leaked = try allocator.alloc(u8, default_page_size + 1);

    var seen_small = false;
    var seen_large = false;
    var it = gpa.iterateLeaks();
    while (it.next()) |entry| {
        try std.testing.expect(entry.addr != @intFromPtr(small_freed.ptr));
        if (entry.addr == @intFromPtr(small_leaked.ptr)) {
            seen_small = true;
            try std.testing.expectEqual(@as(usize, 8), entry.size);
        }
        if (entry.addr == @intFromPtr(large_leaked.ptr)) {
            seen_large = true;
            try std.testing.expectEqual(default_page_size + 1, entry.size);
        }
    }
    try std.testing.expect(seen_small);
    try std.testing.expect(seen_large);

    allocator.free(small_leaked);
    allocator.free(large_leaked);
